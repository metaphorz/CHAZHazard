...
The visualization allows users to explore hurricane hazard under three Shared Socioeconomic Pathways (SSPs), which represent different assumptions about future socioeconomic development and greenhouse gas emissions. Table~\ref{tab:ssp} describes each scenario.
...
\begin{tabular}{llll}
\toprule
\textbf{Scenario} & \textbf{Description} & \textbf{CO$_2$ by 2100} & \textbf{Warming} \\
\midrule
SSP245 & Middle of the road, moderate mitigation & $\sim$600 ppm & $\sim$2.7°C \\
SSP370 & Regional rivalry, medium-high emissions & $\sim$850 ppm & $\sim$3.6°C \\
SSP585 & Fossil-fueled development, high emissions & $\sim$1100 ppm & $\sim$4.4°C \\
\bottomrule
\end{tabular}
\end{table}

The SSP number (e.g., 585) encodes both the socioeconomic pathway (first digit) and the approximate radiative forcing in W/m$^2$ by 2100 (remaining digits). SSP585 represents a future with high energy demand met primarily by fossil fuels, while SSP245 assumes moderate mitigation efforts leading to stabilization.
...
\begin{itemize}
    ...
    \item \textbf{Data Point Tooltips}: Hover over any marker to display coordinates, wind speed (m/s, km/h, mph), hurricane category, and all return period values
